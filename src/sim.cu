...
#ifdef RK2
        computeSpringForces<<<springBlocksPerGrid, THREADS_PER_BLOCK,3*THREADS_PER_BLOCK*sizeof(d_spring)>>>(d_spring, springs.size(), T); // compute mass forces after syncing
        gpuErrchk( cudaPeekAtLastError() );
